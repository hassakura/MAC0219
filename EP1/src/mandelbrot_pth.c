...
void write_to_file_graphic(){
    FILE * file;
    char * filename = "pthread_times.txt";

    file = fopen(filename,"wb");

    fprintf(file, "%lf ", cpu_time_used);

    fclose(file);
}

void compute_mandelbrot_partition(int i_start_y, int i_finish_y){
    ...
    for(i_y = i_start_y; i_y < i_finish_y; i_y++){
...
void *compute_mandelbrot(void *threadid){
    int i_start_x, i_start_y, i_finish_x, i_finish_y;
    double c_start_x, c_start_y, c_finish_x, c_finish_y;
    long tid;
    tid = (long) threadid;
    i_start_y = tid * i_y_max / NUM_THREADS;
    i_finish_y = (tid + 1) * i_y_max / NUM_THREADS;
    compute_mandelbrot_partition(i_start_y, i_finish_y);
    pthread_exit(NULL);
}
...
void create_threads(){
    ...
    for (t = 0; t < NUM_THREADS; t++){
        error_code = pthread_create(&threads[t], NULL, compute_mandelbrot, (void *) t);
        if (error_code){
